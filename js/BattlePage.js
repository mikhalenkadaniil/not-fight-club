...
function getBattleLogHTML() {
    ...
    Критический урон рассчитывается для каждой атаки отдельно (например, у волка будет 2 расчета за ход)<br>
    Противник выберается случайным образом из трех возможных:<br>
    1) Волк[100 хп] (особое умение - атакует 2 раза за ход, но не защищается) <br>
    2) Кабан[150хп] (особое умение - атакует 1 раз за ход и имеет 2 зоны защиты)<br>
    3) Медведь[300хп] (особое умение - атакует 1 раз за ход, но не защищается)<br>
    Игрок имеет первоочередное преимущество в обмене ударов. (например, если в ход и противнику и игроку нанесен смертельный урон - победит игрок, т.к. его удар считается первым)
    </p>` // Log to reviewer
    const state = JSON.parse(localStorage.getItem('battleState'));
